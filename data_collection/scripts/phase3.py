...
class DocumentDownloader:
    ...
    def fetch_with_retry(self, url: str, max_retries: int = RETRY_MAX) -> Optional[bytes]:
    ...
    def rewrite_paths(self, htm_content: bytes, url_to_filename: Dict[str, str], doc_name: str) -> bytes:
        try:
...
class Phase3Pipeline:
    ...
    def run(self):
        ...
        for i, batch in enumerate(self._batch_documents(documents, DOWNLOAD_BATCH_SIZE)):
            for idx, doc in enumerate(batch, 1):
                self._process_document(doc, idx, len(batch))
        
        print(f"Done: {self.stats['downloaded']} docs, {self.stats['images_saved']} images\n")
    ...
    def _load_documents(self, csv_path: Path) -> List[Dict]:
        ...
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                if action in ['download', 'download_with_assets'] and \
                   category in ['indenture', 'image_based_indenture']:
        ...
        if category == 'image_based_indenture':
            ...
            if dependencies:
                ...
                for dep_url in dependencies:
                    ...
                    if dep_content:
                        saved_filename = self.asset_mgr.save_image(dep_url, dep_content, assets_dir)
                        if saved_filename:
                            url_to_filename[dep_url] = saved_filename
                            self.stats['images_saved'] += 1
                
                htm_content = self.downloader.rewrite_paths(htm_content, url_to_filename, doc_name)
